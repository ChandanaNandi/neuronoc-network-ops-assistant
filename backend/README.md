...
- `POST /api/remediation/recommendations/{id}/approve` — record approval intent on a remediation plan. **Phase 23: requires bearer-token auth + role `admin`.** Body carries `{note?}` only — the approving operator's identity comes from the authenticated session, NOT from a submitted field (legacy `operator_id` / `operator_name` body fields now 422 via `extra="forbid"`). 401 if unauthenticated, 403 if authenticated as a non-admin role, 404 if recommendation missing, 400 if recommendation_type ≠ `remediation_plan`. Audit fields (`approved_by`, `approved_by_operator_id`) populated from the bearer-token session. **Records intent only — no execution.**
- `POST /api/remediation/recommendations/{id}/reject` — same auth/RBAC + body contract, marks rejected.
- `POST /api/auth/login` — Phase 23: body `{display_name, password}` → 200 + `{token, operator}`. Generic 401 "invalid credentials" for unknown user / wrong password / operator without a password set (the API never leaks which case fired).
- `POST /api/auth/logout` — Phase 23: invalidates the current bearer-token session. 401 if called unauthenticated.
- `GET /api/auth/me` — Phase 23: returns the operator bound to the current bearer token; 401 otherwise. Used by the frontend to rehydrate login state on page load.
...
- `GET /api/runbooks/rag/search?q=…&incident_id=…&limit=5` — Phase 25: embedding-backed RAG search over chunked Markdown runbooks. Uses FAISS for vector search. Embeddings use the deterministic local hashing backend by default for offline CI/dev; set `RAG_EMBEDDING_BACKEND=sentence-transformers` to use `sentence-transformers` with `RAG_EMBEDDING_MODEL` when a model is available. Returns cited chunks with `{slug,title,score,excerpt,path,chunk_index,citation_id,embedding_backend,embedding_model}`.
- `GET /api/runbooks/rag/evaluate?top_k=3` — Phase 25: runs the bundled retrieval evaluation set and returns top-k accuracy, source coverage, citation coverage, answer-faithfulness proxy, and average latency.
...
- `POST /api/telemetry/observations` — Phase 22A: persist a validated `TelemetryEvent` as a row in the new `telemetry_observations` table. Returns 201 with the persisted row including server-stamped `id` + `received_at`. **No auto-incident creation, no auto-correlation** — `created_incident_id` is always `null` for rows created in this phase; the FK exists so a future phase can link an observation to the incident it produced (`ON DELETE SET NULL` so deleting the incident doesn't cascade-destroy the original observation). The Phase 18A `/validate` and Phase 18B `/correlate/preview` endpoints are **unchanged** and continue to write nothing to the DB — pinned by `test_validate_endpoint_remains_non_persisting_after_phase22a` and `test_correlate_preview_remains_non_persisting_after_phase22a`. 422 on schema failure.
- `GET /api/telemetry/observations?limit=50` — Phase 22A: list persisted observations, newest first (`received_at DESC, id DESC` for stable tie-breaking). Limit 1..100; 422 for out-of-range.
- `GET /api/telemetry/observations/{id}` — Phase 22A: fetch one persisted observation by id; 404 if missing.
- `POST /api/telemetry/observations/{id}/correlate` — Phase 22B: deterministically correlate a persisted observation into an `Incident`. Re-runs the existing Phase 18B `build_correlation_preview` over the stored payload. Three paths: (a) generic-fallback preview → no Incident created, `correlated=false`; (b) observation already linked → returns existing Incident, `incident_created=false` (idempotent); (c) specific match → creates ONE Incident + ONE `IncidentEvent` (using the preview's `suggested_event_type` / `suggested_event_source` / `suggested_event_payload`) and sets `telemetry_observations.created_incident_id` to the new id. Response: `PersistedCorrelationResult` with `{observation_id, correlated, incident_created, incident_id, suggested_incident_type, rationale}`. 404 if observation id unknown. **No LLM, no remediation execution, no device contact** — pinned by a test that monkeypatches `generate_ollama_json` and `build_remediation_plan` to raise on any accidental call.
...
- `app/knowledge/rag.py` — chunks the same runbooks, embeds chunks, builds a FAISS vector index, returns cited chunks, and exposes the small RAG evaluation set. Defaults to deterministic local embeddings; `RAG_EMBEDDING_BACKEND=sentence-transformers` enables Sentence Transformers when the model is available.
...
3. Retrieve up to 3 cited runbook chunks using embedding RAG over `incident_type + key_findings + correlated_signals` as the query.
4. Build the prompt (constraint: *use only the provided evidence and cited runbook snippets*).
